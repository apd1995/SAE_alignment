...
output_directory = os.path.join(input_directory, 'pca_plots_0_1')
...
for layer_idx in range(layers):
    ...
    safe_mean = safe_tensor.mean(dim=1).cpu().numpy()  # Take the mean along dim=1 and convert to numpy
    ...
    unsafe_mean = unsafe_tensor.mean(dim=1).cpu().numpy()  # Take the mean along dim=1 and convert to numpy
    ...
    # Plot the data points along the first two principal components for this layer
