...
classifier_directory = os.path.join(input_directory, 'save_matrix_data/separability')
output_directory = os.path.join(input_directory, 'cosine_similarity_separator')
...
for layer_idx in range(layers):
    ...
    coef_path = os.path.join(classifier_directory, f'layer_{layer_idx}_weights_w.txt')
    coef = np.loadtxt(coef_path)  # shape: (1, feature_dim)
    ...
    cosine_similarity = np.abs(Vt @ coef_norm.T)  # shape: (num_components, 1)
