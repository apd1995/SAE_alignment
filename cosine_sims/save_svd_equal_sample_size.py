...
output_directory = os.path.join(input_directory, 'svd_results_equal_sample_size')
...
safe_mean_len = safe_mean.shape[0]
...
unsafe_mean_len = unsafe_mean.shape[0]

min_len = min(safe_mean_len, unsafe_mean_len)
safe_mean = safe_mean[:min_len, :]
unsafe_mean = unsafe_mean[:min_len, :]
